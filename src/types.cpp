...
void to_json(json& j, const GUY& guy)
{
    ...
    for (size_t i = ROHAST; i <= ROHSCHLEUDER; i++)
    {
        j["Inventar"][std::to_string(i)] = guy.Inventar[i];
    }
}
...
void from_json(const json& j, GUY& guy)
{
    ...
    for (size_t i = ROHAST; i <= ROHSCHLEUDER; i++)
    {
        try
        {
            guy.Inventar[i] = inventory.at(std::to_string(i)).get<short>();
        }
        catch (std::exception&)
        {
...
void to_json(json& j, const BMP& bmp)
{
    ...
    j["Rohstoff"] = {};
    for (size_t i = WASSER; i <= GESUNDHEIT; i++)
    {
...
{
    wav.Dateiname = j.at("Dateiname").get<std::string>();
    wav.Loop = j.at("Loop").get<bool>();
    wav.Volume = j.at("Volume").get<short>();
}
...
{
    j = json{
        {"Aktiv", textbereich.Aktiv},
        {"rcText", textbereich.rcText}
    };
}

void from_json(const json& j, TEXTBEREICH& textbereich)
{
    textbereich.Aktiv = j.at("Aktiv").get<bool>();
    textbereich.rcText = j.at("rcText").get<RECT>();
}
