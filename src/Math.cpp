...
void UpdateMousePosition(short Button, short Push, short xDiff, short yDiff)
{
    ...
    // Info anzeigen
    Coordinate Erg =
        Renderer::GetTile((MousePosition.x + Camera.x), (MousePosition.y + Camera.y)); // Die angeklickte Kachel
...
void MouseInPanel(short Button, short Push)
{
    ...
    else if ((InRect(MousePosition.x, MousePosition.y, Bmp[INVENTORY_PAPER].targetRect)) && (HauptMenue ==
        Menu::INVENTORY))
    {
...
void CalcGuyKoor()
{
    if (Step >= Steps)
    {
        ...
        if (RouteLaenge > 1)
        { // Bei normaler Routenabarbeitung die Richung Kachelmäßig rausfinden
    ...
    if (CurrentFrame % Landscape[Guy.Pos.x][Guy.Pos.y].RunningTime == 0)
    {
        ...
        if ((s_GameState == State::INTRO) || (s_GameState == State::RESCUED))
        { // Beim Intro fährt die Kamera mit
    ...
    if (CreditsState == 0)
    {
        for (short k = 1; k < 10; k++)
        {
            ...
            if (Bmp[CreditsList[CreditsNum][k].Picture].targetRect.top < 0)
            {
                ...
                Bmp[CreditsList[CreditsNum][k].Picture].targetRect.top = MAX_SCREEN_Y - Bmp[CreditsList[CreditsNum][k].
                    Picture].Height / 2;
...
void Animationen()
{
        ...
        (Guy.AnimationState == GUY_SHIP) || (Guy.AnimationState == GUY_SWIM))
    {
        ...
        for (short k = 0; k < loop; k++)
            if ((CurrentFrame % i == 0) && (Guy.IsActive))
            {
                CalcGuyKoor();
            }
